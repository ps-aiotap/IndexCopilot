...
st.set_page_config(
    page_title="IndexCopilot - Portfolio Manager",
    page_icon="📊",
    layout="wide"
)
...
st.markdown("### Portfolio Manager for Indian Index Funds")
st.markdown("---")
...
with tab1:
    ...
    # Portfolio name
    portfolio_name = st.text_input("Portfolio Name", value=st.session_state.portfolio["name"])
    ...
    if st.session_state.portfolio["holdings"]:
        ...
        ax.pie(asset_allocation, labels=asset_allocation.index, autopct='%1.1f%%', startangle=90)
        ax.axis('equal')
        st.pyplot(fig)
    ...
    if method == "Upload CSV":
        ...
        **CSV Format Example:**
        ...
        if uploaded_file is not None:
            try:
                ...
                if all(col in holdings_df.columns for col in required_columns):
                    ...
                    st.error("CSV must contain asset_type, asset_id, asset_name, quantity, and purchase_price columns")
            except Exception as e:
        ...
        with st.form("add_holding_form"):
            asset_type = st.selectbox("Asset Type", ["mutual_fund", "equity", "insurance"])
            ...
            purchase_price = st.number_input("Purchase Price (₹)", min_value=0.0, value=100.0, step=1.0)
...
# Footer
st.markdown("---")
st.caption("IndexCopilot - Portfolio Manager for Indian Index Funds")